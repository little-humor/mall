layui.use(['table','layer','form','upload','carousel','colorpicker'], function(){
    ...
    var url_prefix = 'http://' + document.domain +':'+ document.location.port+ "/admin/";
    ...
    var renderTable = table.render({
        ...
        height: 'full',
        even: true,//隔行背景
        url: url_prefix+'product/list.do', //数据接口
        // where: {},
        page: true, //开启分页
        cols: [[ //表头
    ...
    upload.render({
        elem: '#upload', //绑定元素
        url: url_prefix + 'common/uploadImages.do', //上传接口
    ...
    table.on('tool(product)',function (obj) {//注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'detail'){ //查看
            ...
            layer.confirm('是否要删除', function(index){
                ...
                $.ajax({
                    method: 'delete',
                    url: url_prefix + 'product/deleteProducts.do',
                    data: {
                        id: data.id
                    },
    ...
    form.on('select(searchCategory)', function(data){
        ...
        $.ajax({
            url: url_prefix+'product/findChildCategory.do',
            data: {
                categoryId: value
            },
            success:function (result,status,xhr) {
    ...
    form.on('submit(submit)',function (data) {
        ...
        $.ajax({
            method: 'POST',
            url: url_prefix + 'product/addOrUpdate.do',
            data: fields,
            success: function (result,status,xhr) {
                layer.msg(result.msg);
    ...
    //异步加载商品分类
    $.ajax({
        url: url_prefix+'product/findParentCategory.do',
        success:function (result,status,xhr) {
            var data = result.data;
